...
function compile(folderName,fileName,timeout=60000){
	...
	const cmd = `docker run -i -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp openjdk:8 javac ${fileName}.java`;
...
function run(folderName,fileName,input,timeout=10000){
	...
	const cmd = `docker run -i -v  "$PWD"/${folder_path}/:/usr/src/myapp -w /usr/src/myapp openjdk:8 timeout --preserve-status 1 java -Xmx1024M -Xms128M ${fileName} < "$PWD"/${input_path}`;
	const result = shell.exec(cmd);
	const jsonResp = {};
	console.log(result)
	if(result.code===1){
